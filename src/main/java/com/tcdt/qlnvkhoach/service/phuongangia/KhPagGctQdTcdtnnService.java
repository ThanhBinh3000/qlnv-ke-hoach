...
public class KhPagGctQdTcdtnnService extends BaseService {
    ...
    public List<KhPagGctQdTcdtnn> dsToTrinhQdTcdtnn(KhPagGctQdTcdtnnSearchReq objReq) throws Exception {
        List<KhPagGctQdTcdtnn> data = khPagGctQdTcdtnnRepository.dsToTrinhTh(objReq.getTrangThai());
        List<Long> idsQdTcdtnn = data.stream().map(KhPagGctQdTcdtnn::getId).collect(Collectors.toList());
        List<KhPagQdTcdtnnCtiet> lChiTiets = khPagQdTcdtnnCtietRepository.findByQdTcdtnnIdIn(idsQdTcdtnn);
        List<String> maDvis = lChiTiets.stream().map(KhPagQdTcdtnnCtiet::getMaDvi).collect(Collectors.toList());
        ...
        data.forEach(dt -> {
            ...
            dt.setKhPagQdTcdtnnCtiets(mapListChitiet.get(dt.getId()));
        });
        return data;
    }
