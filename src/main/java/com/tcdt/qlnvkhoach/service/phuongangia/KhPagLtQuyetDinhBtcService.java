...
@Service
@Log4j2
public class KhPagLtQuyetDinhBtcService {
  @Autowired
  ...
  public KhPagLtQuyetDinhBtc create(CustomUserDetails currentUser, KhPagLtQuyetDinhBtcReq req) throws Exception {
    ...
    newRow.setTrangThai(PAGTrangThaiEnum.DU_THAO.getId());
    newRow.setMaDonVi(currentUser.getDvql());
    newRow.setCapDvi(currentUser.getUser().getCapDvi());
    newRow.setNguoiTaoId(currentUser.getUser().getId());
    newRow.setNguoiSuaId(currentUser.getUser().getId());
    return khPagLtQuyetDinhBtcRepository.save(new KhPagLtQuyetDinhBtc());
  }
