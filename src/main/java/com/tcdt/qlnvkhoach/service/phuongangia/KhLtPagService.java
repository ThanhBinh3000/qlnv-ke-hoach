        ...
        try {
            ...
            if (listPagCCPhapLy.size() > 0) {
                ...
                listPagCCPhapLy.forEach(f -> {
                    f.setFileDinhKem(mapListFile != null && mapListFile.size() > 0 && mapListFile.get(f.getId()).size() > 0 ? mapListFile.get(f.getId()).get(0) : null);
                });
            ...
            if (listPagKetQuaTD.size() > 0) {
                listPagKetQuaTD.forEach(f -> {
                    f.setTenCloaiVthh(StringUtils.isEmpty(f.getCloaiVthh()) ? null : hashMapHh.get(f.getCloaiVthh()));
                    f.setFileDinhKem(( mapListFileForKqs.size() > 0 && mapListFileForKqs.get(f.getId()) != null && mapListFileForKqs.get(f.getId()).size() > 0 ) ? mapListFileForKqs.get(f.getId()).get(0) : null);
                });
