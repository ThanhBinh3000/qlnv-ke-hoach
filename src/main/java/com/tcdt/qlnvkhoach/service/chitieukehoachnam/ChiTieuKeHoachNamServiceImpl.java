...
import com.tcdt.qlnvkhoach.request.object.chitieukehoachnam.*;
...
//		}
  ...
  private void validateXuatTrongNam(VatTuNhapReq xtnReq, TonKhoDauNamRes tonKhoDauNam) throws Exception {
    try {
      ...
      if (tonKhoDauNam == null && DataUtils.safeToDouble(xtnReq.getSoLuong()) > 0) {
        ...
        if (CollectionUtils.isEmpty(tonKho) && DataUtils.safeToDouble(xtnReq.getSoLuong()) > 0) {
          throw new Exception("Xuất trong năm không được lớn hơn tồn đầu năm.");
        }

        VatTuNhapRes tkdn = tonKho.stream().filter(tk -> tk.getNam().equals(nam)).findFirst().orElse(null);
        if ((tkdn == null && DataUtils.safeToDouble(xtnReq.getSoLuong()) > 0) || (tkdn != null && DataUtils.safeToDouble(xtnReq.getSoLuong()) > DataUtils.safeToDouble(tkdn.getSoLuong()))) {
          throw new Exception("Xuất trong năm không được lớn hơn tồn đầu năm.");
        }
      }
    } catch (Exception e) {
